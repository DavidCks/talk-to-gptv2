...
export function setButtonHasPlayedInitially(
  target: HTMLElement,
  value: "true" | "false"
) {
  const btns = target.getElementsByClassName("play-button")
...
function getCurrentLabel(target: HTMLElement): string {
  ...
  if (btns.length == 0) {
    ...
    console.log(
      "Every element should have only one play button element inside it. The offending item was:"
    )
    console.log(target)
  }
...
export function build(
  target: HTMLElement,
  { shouldPlay = false, canPlay = false }
...
function noSpeechApiError() {
  alert(
    "The \"Talk to GPT\" extension currently doesn't support browsers without a native SpeechSynthesis API. For now you'll need to use a modern browser or update your current one. You can find a list of supported browsers here: https://caniuse.com/speech-synthesis"
  )
}
...
function playUtterances(
  utterances: [SpeechSynthesisUtterance],
  utterance: SpeechSynthesisUtterance | null = null,
  { target: target, btn: btn }
) {
  ...
  if (utterances.length > 0) {
    ...
    utterance.onend = () => {
      ...
      if (nextt !== null && nextt !== undefined) {
        const nextb: HTMLButtonElement = nextt.getElementsByClassName(
          "play-button"
