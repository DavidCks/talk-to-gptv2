...
function observe(el: HTMLElement) {
    ...
    const observer = new MutationObserver(async (mutations) => {
        ...
        paragraphs.forEach(async (p) => {
            ...
            const dialogueIsTrue = document.querySelector("#controls-dialogue_toggle") !== undefined && document.querySelector("#controls-dialogue_toggle").getAttribute("aria-checked") == "true"
            const playButton = build(p as HTMLElement, { canPlay: !targetHasAfter, shouldPlay: dialogueIsTrue })
            if (playButton !== null) {
                const existingButton = p.getElementsByClassName("play-button")[0]
                if (existingButton !== undefined && existingButton.getAttribute("rendered") === "false") {
                    p.removeChild(existingButton)
                } else if (existingButton === undefined || existingButton.getAttribute("rendered") === "false") {
...
function isTargetControlsMutation(target: HTMLElement): boolean {
    ...
    if (target.parentElement.id == "textarea_wrapper_root" || target.parentElement.tagName == "FORM" || target.tagName == "TEXTAREA") {
        return true
