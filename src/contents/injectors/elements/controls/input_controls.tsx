...
import type { SpeechRecognitionOptions, ListeningOptions } from "react-speech-recognition"
...
import cssText from "data-text:~contents/injectors/elements/controls/input_controls.css"
import { getGlobalClassName, injectStyle } from "~contents/utils/styler"
import { injectPromptsAndCancelReading } from '../../../manipulators/events';
injectStyle(cssText)
...
export const InputControls = () => {
    ...
    if (!browserSupportsSpeechRecognition) {
        return <span className={classList() + globalClassName}>Your Browser doesn't support speech recognition.</span>;
    }

    if (!isMicrophoneAvailable) {
        return <span className={classList() + globalClassName}>Please allow microfone access to use voice recognition.</span>;
    }
    ...
    const handleLangSelectChange = (e) => {
        ...
        const langCode: string = supportedLanguages.filter(([label, _dialect, _langCode]) => {
    ...
    const textareaElement: HTMLTextAreaElement = getInjectorRootElement() as HTMLTextAreaElement
    useEffect(() => {
        if (transcript !== "") {
            ...
            {(supportedLanguages
                ...
                .map(([label, dialect, langCode]) => (
                    (dialect as string[]).map((dialectLabel, i) => (
                        <option key={dialectLabel as string}
